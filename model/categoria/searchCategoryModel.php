...
class SearchCategoryModel extends Database{
    ...
    function getSearch($search, $paginaActual){
        ...
        $query = $this->connect()->prepare("SELECT COUNT(*) AS total FROM article WHERE category LIKE :name");
        $query->execute(['name'=>$search]);
        $totalPaginas =  ceil($query->fetch(PDO::FETCH_OBJ)->total / $resultadosMostrados);
        
        $query = $this->connect()->prepare('SELECT * FROM article WHERE category LIKE :name LIMIT :inicioArticle, :finDeArticle');
        ...
        if($query->rowCount()){
            ...
            for($e=$inicioPagination; $e<=$finPagination; $e++){
                ...
                if($e>0 && $e<$totalPaginas+1){
                    ...
                    if($e==$paginaActual){
                        echo '<a href="'.URL.'categoria/'.$search.'/'.$e.'" class="paginaSeleccion">'.$e.'</a>';
                    }else{
                        echo '<a href="'.URL.'categoria/'.$search.'/'.$e.'">'.$e.'</a>'; 
                    }
                }
            }
